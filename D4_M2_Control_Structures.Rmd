...
purrr???? - JP is covering on Day 6. Introduce here?

Thomas - put together a small sample dataset of CSV hobo files for reading in and manipulating using iteration.

####control structures

What are control structures? Lots of great analogies here, but I will spare you. The simplest way to think about this is that these are programming techniques that give your code flexibility. Your code can do different things depending on the data it encounters - this is very useful for loops/iteration, which we will talk about next week. There are a variety of different control structures, but we are going to focus on the most common one, the if/else statement. Most of you are probably familiar with this. It has 3 pieces (in order):
...
```{r Day4Recode.1, echo=TRUE, eval=TRUE}
...
```{r Day4Recode.2, echo=TRUE, eval=TRUE}
d<-c(1,2,3,4,100,200,300,400,NA)
ifelse(d<mean(d, na.rm=T)/10, "verify","OK")
help(ifelse) #so no surprises there...
```

That ```NA```, gets passed through. Maybe we want that, but if we don't we can try to have two 

```{r Day4Recode.3, echo=TRUE, eval=TRUE}
...
First thing to know about the if...else construction in R that it is very sensitive to the bracket placement, else must always be between a set of open brackets like this ```}else{``` . If it isn't you will end up with errors that are hard to track down. So. In context:
...
This statements expects a single boolean in the if() statement. Sometimes I like to use this to create switches or bypasses when I am writing loops/iteration. For example: If I have a dataframe that is non-empty, then do the analysis. If the data frame is empty do something else but don't give up! 

```{r Day4Flagging.1, echo=TRUE, eval=F}
...
if("statement 1"){"Thing to do if S1 is true"
        }else if("statement 2"){"Thing to do if S2 is true"
        }else{"thing to do for everything else"
      }
...
This brings us to iteration. Iteration allows you to a lot of work without having to be the one doing it. This is mostly a good thing. Iteration consists of a few pieces:
...
There are a lot of ways to accomplish iteration and we are not going to cover them all. We are gong to cover some of the most basic and commonly used approaches.
...
i - i is an arbitrary variable, you could call it q. I will take on the value for the first item in the vector on the first cycle thorugh the tasks, it will then move to the next one and go through again, and so on.

vector - vector is that vector of items. These could be numeric or they could be characters or strings. Numeric vectors are common is you need to run through all of the cases. Character vectors are great if you have groups of data you want to work through. All vector does it gives it some instructions on what it should work on and effectively when to stop.

```{r ForLoop.1, echo=TRUE, eval=TRUE}

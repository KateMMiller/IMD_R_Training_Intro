...
Note that I'm using 6-digit hex colors (i.e., "#ffffff" is white) to define the fill color for each habitat type. To find your own or see what color these look like, check out <a href="https://htmlcolorcodes.com/">htmlcolorcodes.com</a>
...
wetsum_sf <- st_as_sf(wetsum, coords = c("X_Coord", "Y_Coord"), crs = 26919) 
  # ACAD is UTM Zone 19N NAD83, hence the different EPSG than SARA, which is 18N.
...
tmap_save(sara_map, "SARA_GRTS.png", height = 10.5, width = 8, units = 'in')
```
<br>
Here's what sara_map looks like in the plot viewer:
```{r, plotvw, echo = T, out.width = '50%', message = F, warning = F}
sara_map
```
<br>
<br>
Here's what the map looks like after it's written to disk and the dimensions are set using `tmap_save()`:
```{r, discvw, echo = F, results = 'show', out.width = '50%'}
knitr::include_graphics("SARA_GRTS.png")
```
</details>
<br>

<details open><summary class='drop2'>Interactive mode in `tmap`</summary>
...
<li>There are all kinds of bells and whistles you can add to a leaflet plot, including popups on hover or click, custom colors and symbols based on the data, etc. For more examples, check out <a href="http://rstudio.github.io/leaflet/">Leaflet for R</a>.</li>
</ul>
...
<a href="http://rstudio.github.io/leaflet/">Leaflet for R</a> covers a lot of the mroe common features for making leaflet maps in R. 

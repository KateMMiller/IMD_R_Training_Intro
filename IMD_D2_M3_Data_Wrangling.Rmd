...
We'll cover the following commands: 
  ...
  3. arrange() - arranges a tibble by column values
  4. filter() - filters data for observations that meet specific criteria
  5. mutate() - adds new column(s) to a tibble
...
select() - keeps the columns you indicate and drops the rest. you can rename the columns you select if you like. Since select() puts the columns in the order you type them, you can also use select() to reorder columns.
rename() - keeps all the columns, but renames the ones you indicate

Since we won't be using geographical information, other than state, in our module, we'll select columns of interest today.

Note that, the name to the left of the arrow is the resulting tibble/df, the information to the right that's connected by pipes are the steps to get to the result. I normally name the result something different than the original tibble/df so that the original data aren't affected. However, since we won't need the geographical information later on, I will name this one "park_visits". 
...
If we take a look at the park_visits data, there are some column name inconsistencies. For example, some are in snake case (unit_code), but some are not (parkname). To rename a column, use rename(). The structure of this argument is as follows: 
...
<br>
<details open><summary class='drop'> dplyr- first challenge</summary>

Time for the first challenge. Using the park_visits data, create a new tibble/df containing total visitation for a state of your choice. Note that some "states" aren't states. For example, VI refers to the Virgin Islands. How can you check your work to make sure your work is correct?

</details>
<br>
<details open><summary class='drop'> dplyr- first challenge solution</summary>
...
state_n2 <- park_visits %>%
           # group by state
           group_by(state) %>%
           # get tally of observations by state in tibble output
           tally()
...
JP - I use group_by() %>% tally() to get a similar result in tibble format. Is there an advantage to vector format? Let me know what you think.
...
This is helpful when you want to get group summary information without sacrificing the original values. For example, if we want to get the total number of visitors per state as a column added to the data, but we don't want to lose individual park visitation numbers. 

```{r eval = TRUE, include = TRUE}
park_visits2 <- park_visits %>%
                # do calculations by state
                group_by(state) %>%
                # add state_totals column with the sum of visitors by state
                # the na.rm = TRUE means that NA values are not included in calculations
                mutate(state_totals = sum(visitors, na.rm = TRUE))
...
For this second challenge, you'll start with the park_visits data. Calculate the average visitation divided by the number of parks for each state. The final df/tibble should be arranged in descending order by a numeric column of your choice. 

...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
# NOTE: We can set aesthetics to constant values by excluding the `aes()` function. For example, `aes(fill = unit_type)` groups and fills histograms based on the `unit_type` variable (different fill color for each unit type). In contrast, `fill = "red"` creates a single histogram that is red.
...
<div class="alert alert-info">
  <strong>CHALLENGE:  Create a stacked bar chart that shows the total number of visitors in each combination of region and year.</strong>
  
  1. Subset the data to keep data only for years 2000 and 2015
  2. Create a basic stacked bar chart faceted by region (3 columns, one region per column). In each facet panel, you should have two stacked bar charts, one for 2000 and one for 2015. The bar heights should represent total visitors summed across park units, with a different color for each unit type.
  
  Hint: map the `x` aesthetic to `year` as a factor, i.e., `x = as.factor(year)`. This isn't absolutely necessary, but will avoid some errors later when we build on this basic bar chart. Bar charts are not meant to display two continuous variables, so with `as.factor(year)` we are making it clear that `year` should be considered a categorical variable.
  </div>
  
The final plot should show this relationship:
```{r, echo = FALSE, message = FALSE, warning = FALSE}

# Subset the data, only keeping data for the years 2000 and 2015
bar_dat <- viz_dat %>%
  dplyr::filter(year %in% c(2000, 2015)) %>%
  dplyr::mutate(year_f = as.factor(year))

head(bar_dat) # always check the data before plotting

# Stacked bar chart, using the handy `weight` argument
ggplot(bar_dat, aes(x = year_f, fill = unit_type)) +
  geom_bar(aes(weight = visitors)) +
  facet_grid(cols = vars(region))

# Compare the plot to the data to make sure things look about right


# NOTE: It's possible to wrangle the data set to calculate the bar heights, instead of using the `weight` argument in `geom_bar`
```

NOTE: Notice that the order of levels in the legend is alphabetical from top to bottom, and the order of colors in the stacked bar chart matches the legend order. If `unit_type` were a factor data type (instead of character), then the legend and chart colors would follow the factor level orders (remember that we had manually set factor level orders for `region`)
...
<div class="alert alert-info">
  <strong>CHALLENGE:  Finalize your bar chart by converting the y-axis values to percentages, adding appropriate plot labels, and making any other tweaks you would like. Save the resulting plot as a .pdf file.</strong>
  </div>
...
(final_bar <- ggplot(bar_dat, aes(x = year_f, fill = unit_type)) +
  geom_bar(aes(weight = visitors), position = "fill") +
  ...
  ggthemes::theme_pander() +
    theme(plot.margin = margin(rep(15, 4)), # increase white margin around plot. May not need this for other themes.
          plot.title = element_text(hjust = 0.5),
          plot.subtitle = element_text(hjust = 0.5)) +
  facet_grid(cols = vars(region)) # Could also do facet_wrap with one row
)
ggsave(filename = "final_bar.pdf", plot = final_bar) # uses extension to figure out file type. Can set width, height and unit or use default based on how it looks on plots page. It will tell you the size it saves at, and you can add that into the script to make it reproducible
```
</details>
<br>
<details open><summary class='drop2'>Further improvements?</summary> 
...
<details open><summary class='drop2'>Step 1. Basic Heat Matrix</summary>
...
<br>
<details open><summary class='drop2'>Step 2. Label cells with their values.</summary>
...
<br>
<details open><summary class='drop2'>Step 3. Make text color conditional on value.</summary>
...
<br>
<details open><summary class='drop2'>Step 4. Finalize the heat map.</summary>

...
```{r load_data1, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}
...
```{r load_data2, echo = TRUE, eval = FALSE, warning = FALSE, message = FALSE}
...
[Reference documents]("http://r-statistics.co/ggplot2-cheatsheet.html") and [cheatsheets](https://nyu-cdsc.github.io/learningr/assets/data-visualization-2.1.pdf") can be very helpful while you are learning `ggplot()`.
...
<details open><summary class='drop2'>Example of aesthetics set in the geometry layer</summary>
...
```{r p1_step2d, echo = TRUE, eval = FALSE, warning = TRUE, message = FALSE}
# Example of setting aesthetic mappings within the layer only
ggplot(data = arch_dat) +
  geom_line(aes(x = year, y = visitors/1000)) +
  geom_point()
...
Use `theme()` to customize all non-data components of the plot. Conveniently, `ggplot()` provides [complete themes](https://ggplot2.tidyverse.org/reference/ggtheme.html) that modify many element themes at once. The default complete theme is `theme_grey`. See `?theme` for all the ways we can modify the non-data plot elements. This help page also provides a hyperlink to a help page for complete themes (I'm sure there is a way to get there more directly, but I couldn't figure it out).

<details open><summary class='drop2'>Explore `ggplot()` complete themes</summary>
...
```{r p6_step1, echo = TRUE, warning = FALSE, message = FALSE}
# NOTES:

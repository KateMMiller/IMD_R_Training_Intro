...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! add some code to introduce dataframe . 
str 
head


```{r ForLoop.2, echo=TRUE, eval=F}
library(magrittr)
library(ggplot2)
fNames <- paste0(
...
for (i in fNames) {
  ...
  fName <- basename(i) # extract the filename from a filepath
  ...
  if (nrow(d) > 0) {
    ...
    p <- ggplot(d, aes(x = DateTime2, y = T_F)) +
      ...
      ggtitle(fName)
    ggsave(
      filename = gsub(".csv", "_plot.pdf", fName), path = OutPath, device = "pdf",
      plot = p,
...
`apply` - Going back to that definition, when might you use it?
 ...
 !!!!!! print d to the console so people can look at what they are woring on.
...
OutDat <- lapply(HoboList, function(x) {
  data.frame(
    gmean = mean(),
    gmin = min(),
    gmax = max(),
...
```{r ApplyFamily.6, echo=TRUE, eval=F}
...
```{r ApplyFamily.7, echo=TRUE, eval=F}
library(ggplot2)
lapply(HoboList, function(x) {
  p <- ggplot(x, aes(x = DateTime, y = T_F)) +
    ...
    ggtitle()
  ggsave(
    filename = , path = , plot = p, device = "pdf",
    width = 5, height = 5, units = "in"
  )
...
```{r ApplyFamily.8, echo=TRUE, eval=F}

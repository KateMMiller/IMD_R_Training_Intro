...
`ifelse()` is the 'vectorized' version - this is most useful when you need to change the value in a vector based i.e. (recoding).
...
0. We are iterating based on the vector `fNames`. There will be as many iterations as there are items in `fnames`. In our case `fNames` is a vector of file paths. This means that on each iteration, `i` will become a different file path. 

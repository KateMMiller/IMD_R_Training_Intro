...
What are control structures? The simplest way to think about this is that these are programming elements that control the flow of data in your code dependng on the value of logical conditions. There are two control structures we are going to focus on: `for` loops and `if/else` statements. `for` loops are a control structure that sets the number of times and the data code should perform an operation on. `if/else` statements control what code is run under different conditions. 
...
i - i is an arbitrary variable, you could call it q. I will take on the value for the first item in the vector on the first step of iteration. q is then available in the loop as a variable and it takes on each successive value in vec with each step of the iteration. 
...
We are going to work through a basic for loop script that mimics some things you might have to do repeatedly. These are raw hobo air temperature files from different sites in the same year. 

```{r ForLoop.2a, echo=TRUE, eval=F}
library(magrittr)
library(ggplot2)
...
```
Note, `paste0()` is an example of a handy vectorized operation. 

Let's get some idea of what we are dealing with so we have a general idea of the code we need to write.
```{r ForLoop.2b, echo=TRUE, eval=F}
#read in and inspect the first element (for training purposes)
temp<-read.csv(fNames[1], skip = 1, header = T)[1:3] #hobo data has an extra line at the top we need to skip
head(temp)
```

```{r ForLoop.2c, echo=TRUE, eval=F}
str(temp) #look at data types
rm(temp) #do a little cleanup 
```
There are ~5 columns in most of them and they all have awful headers... raw data... what are you going to do... R... that is what! 
  ...
  + Col. 5 is a comment column
  
We are only interested in the first 3 columns.

So, 3 columns of data, two columns are numeric/integer, one is character. Really ugly headers that need cleanup. Also because I know these data, one of the files is empty except for the headers.
...
In a `for` loop, you decide ahead of time how many times the code inside your loop should execute. The `for` loop iterates through a vector of a certain length and stops when it reaches the end. This is useful because having a vague criteria for ending iteration could end up with you code runnging a very long or infinite loop. 
